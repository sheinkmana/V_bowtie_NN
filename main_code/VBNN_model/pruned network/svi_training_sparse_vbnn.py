...
from ..training_vbnn import VBNN_SVI_improving
from .prediction_sparse_vbnn import VBNNSparsePredictionMixin


class VBNN_SVISparseTraining(VBNNSparsePredictionMixin):
    """
    VBNN training with SVI and fixed sparsity pattern.
    
    Continues training only active weights/neurons identified through FDR-based
    sparsification. Inherits training logic from VBNN_SVI_improving but operates
    ...
    def from_pretrained(
        cls,
        full_model: 'VBNN_SVI_improving',
        alpha: float = 0.01) -> 'VBNN_SVISparseTraining':
        """
    ...
    def _create_sparse_instance(
        ...
        sparse_model = object.__new__(VBNN_SVISparseTraining)
    ...
    def _copy_from_full_model(
        ...
        # Sample handling
        self.N = full_model.N
        if full_model.sample_size:
            self.sample_size = full_model.sample_size
            self.sample_indices = full_model.sample_indices.copy()
            self.x_sample = self.x[self.sample_indices]
            self.y_sample = full_model.y_sample.copy()
    ...
    def _prune_activation_params( self,
        ...
        self.rho = []
        self.A = []
        batch_size = self.sample_size if self.sample_size else self.N
        ...
        for k in range(self.L):
            ...
            S_k_indices = np.ix_(np.arange(batch_size), active_curr, active_curr)
        ...
        # Cached activations
        self.cached_means = [
            np.zeros((batch_size,  len(active_neurons[i]), 1)) for i in range(self.L + 1)
        ]
        self.cached_aats = [
            np.zeros((batch_size,  len(active_neurons[i]), len(active_neurons[i]))) 
        for i in range(self.L + 1)
        ]
        self.cache_valid = False

        # Prediction structures (initially None, created when needed)
        self.M_predict = None
        self.b_predict = None
        self.S_predict = None
        self.rho_predict = None
        self.A_predict = None
        self.cached_means_predict = None
        self.cached_aats_predict = None
    ...
    def _compute_forward_pass(self):
        ...
        x_data: np.ndarray
        if self.sample_size and self.x_sample is not None:
            x_data = self.x_sample
        else:
            x_data = self.x
        
        # Input layer
        self.cached_means[0] = x_data  # Shape: (batch_size, D[0], 1)
        self.cached_aats[0] = np.array([x @ x.T for x in x_data])
        
        # Hidden layers
        for k in range(self.L):
            means = np.zeros((batch_size, self.D[k+1], 1))
            aats = np.zeros((batch_size, self.D[k+1], self.D[k+1]))
    ...
    def update_global_params(self, weights = True, weightsout = True, eta = True, forgrate = 0.75):
        ...
        for j in range(self.D[self.L+1]):
            ...
            intermed_b = (1- learning_rate)*solve(self.big_b[self.L][j], np.eye(self.D[self.L]+1), assume_a='pos') + learning_rate*(np.diag(np.array([1/self.s_0**2] + [inv_mean_IG(self.nu_glob[self.L], self.delta_glob[self.L])*inv_mean_IG(self.nu_loc[self.L][j][i], self.delta_loc[self.L][j][i]) for i in range(self.D[self.L])]))\
    ...
    def _elbo_agamma(self):
        ...
        if self.sample_size:
            ...
            if self.y_sample is None:
                raise ValueError("y_sample is not here")
            else:
